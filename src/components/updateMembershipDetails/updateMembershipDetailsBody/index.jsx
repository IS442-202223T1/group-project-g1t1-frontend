  ...
  return (
    <div className="w-full bg-white rounded-lg border shadow-md">
      <ul className="flex flex-wrap items-center justify-between text-sm font-medium text-center bg-gray-50 rounded-t-lg border-b border-gray-200">
        <div className="flex">{renderButtonTabs}</div>
        <div className="flex">
      ...
      {activeTabIndex === 0 && (
        <AdminContent
          ...
          fee={membershipDetails.replacementFee}
          isElectronicPass={membershipDetails.isElectronicPass}
          emailTemplate={
            membershipDetails.emailTemplate === null ||
            membershipDetails.emailTemplate.templateContent === null
              ? ""
              : membershipDetails.emailTemplate.templateContent
          }
          attachmentTemplate={
            membershipDetails.attachmentTemplate === null ||
            membershipDetails.attachmentTemplate.templateContent === null
              ? ""
              : membershipDetails.attachmentTemplate.templateContent
          }
          membershipGrade={membershipDetails.membershipGrade}
          logoUrl={membershipDetails.logoUrl}
        />
      )}
      {activeTabIndex === 1 && (
        <PassTableContent
          passes={membershipDetails.corporatePasses}
          isElectronicPass={membershipDetails.isElectronicPass}
        />
      )}
    </div>
  ...
  return (
    <button
      ...
      className={`inline-block p-4 ${
        activeTabIndex === index ? "text-redPri" : "text-gray-600"
      } hover:bg-gray-100`}
    >
      {buttonName}
    </button>
          ...
          <tbody>
            {passes.map((pass) => (
              <tr className="bg-white divide-y">
                ...
                {isElectronicPass ? <td className="py-4 px-6">{pass.expiryDate}</td> : null}
  ...
  return (
    <>
      ...
      <li className="py-3 sm:py-4">
        <div className="flex items-center space-x-4 justify-start">
          <div className="flex-none w-44">
            <p className="text-sm font-medium text-gray-900 truncate dark:text-white">Logo URL</p>
          </div>
          <div className="flex-1 items-center text-base font-semibold text-gray-900 dark:text-white">
            {logoUrl}
          </div>
